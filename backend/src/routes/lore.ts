...
import { requireRpTier } from '../middleware/tier.js';

const router = Router();

// Get all lore entries for a guild, optionally filtered by tag
router.get('/guild/:guildId', requireRpTier, async (req, res) => {
  try {
...
router.post('/', requireRpTier, async (req, res) => {
  try {
...
router.post('/channel/tag', requireRpTier, async (req, res) => {
  try {
...
// Delete a lore entry
router.delete('/:id', requireRpTier, async (req, res) => {
  try {
    const { id } = req.params;
